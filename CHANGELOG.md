...
- A card's build signature now digests each referenced neighbour's date, authors and DOI alongside its title and taxon, because `@mention` in `@note` calling card renders those fields.
...
- **Breaking:** `style.css`, `katex.min.css`, and the fonts their `@font-face` rules point at (Inria Sans, KaTeX's own) are now bundled with `tr` and written to the output directory on every build, overwriting any same-named file a site's assets directory provides — both stylesheets are pinned to what `tr` generates (the taxon-numbering counters above; `katex-stdio-deno.ts`'s `npm:katex@0.18.1`), and a site's copy drifting out of sync broke math layout in practice. The bundled `style.css` drops rules for markup `tr` doesn't generate — the search dialog and Agda's `.Agda`-prefixed syntax-highlight classes — since keeping those correct was never `tr`'s job. When a site's own copy of a bundled file differs, the build prints the `site.rkt` `'head` snippet that keeps its extra rules applying (rename the file, then link it); identical files are left silent.
