...
@p{@code{tr} use @a['href: "https://katex.org/"]{Katex} in two forms: @code["@m{...}"] and @code["@mm{...}"]. One for inline and one for block, e.g. @code["@m{a \\in \\R}"] produces @m{a \in \R}; @code["@mm{ \\int_{a}^{b} x^2 \\,dx }"] produces}
@mm{ \int_{a}^{b} x^2 \,dx }
